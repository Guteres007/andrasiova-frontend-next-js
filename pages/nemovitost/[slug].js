...
export async function getServerSideProps(context) {
    ...
    const coverImage = property.data.medias.filter(
        (image) => image.pivot.role === 'cover' && image.pivot.crop === 'default'
    )[0]
    const images = property.data.medias
        .filter((image) => image.pivot.crop === 'default')
        .map((image) => {
            /*  {
    ...
    return {
        props: { nextProperties: nextProperties.data, property: property.data, images, coverImage },
    }
}
                            ...
                            <div className={'property-carousel'}>
                                <ImageGallery
                                    ...
                                    renderCustomControls={() => {
                                        return (
                                            <div className={styles.propertyTitle}>
                                                <h1 className={styles.propertyH1}>
                                                    {property.title}
                                                </h1>
                                            </div>
                                        )
                                    ...
                                    renderLeftNav={(onClick, disabled) =>
                                        leftNavigation(onClick, disabled)
                                    }
                                    renderRightNav={(onClick, disabled) =>
                                        rightNavigation(onClick, disabled)
                                    }
                                />
                            </div>
                        </Col>
                    ...
                    <Row>
                        ...
                        <Col xl={5}>
                            <div className={styles.imageSection}>
                                <div className={styles.rightBackgroundOffset}></div>
                    ...
                    <Container>
                        <Row>
                            ...
                            <Col xl={12} className={styles.relatedPropertiesButton}>
                                <Button>Načíst další nemovitosti</Button>
                            </Col>
                        </Row>
                    </Container>
                </div>
